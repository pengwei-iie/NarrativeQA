...
from tf_utils.recurrent_layers import CudnnLstm, CudnnGru
...
def bi_rnn_encoder(cell_type, hidden_size, num_layers, keep_prob, inputs,
    ...
    if input_lengths != None:
        outputs, final_state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, inputs, input_lengths, dtype=tf.float32)
    else:
        outputs, final_state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, inputs, dtype=tf.float32)
