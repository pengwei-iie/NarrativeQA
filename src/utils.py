...
def eval_set(predictions, ground_truth):
    ...
    word_target_dict = dict(enumerate(map(lambda item: map(lambda s: " ".join(s), item), word_target)))
    word_response_dict = dict(enumerate(map(lambda item: [" ".join(item)], word_response)))
    ...
    #     bleu1_scores, bleu4_scores, meteor_scores, rouge_scores, cider_scores
    return bleu1_score, bleu4_score, None, rouge_score, cider_score, \
           bleu1_scores, bleu4_scores, None, rouge_scores, cider_scores
