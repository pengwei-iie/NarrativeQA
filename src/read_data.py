...
def parse_raw_wikihop_data(path, tfidf_layer, only_follows = False):
    ...
    with open(path) as input_json:
        ...
        for id, row in tqdm(enumerate(reader)):
            ...
            #print row
            question = lower_all(row['query'].replace("_", " ").split(" "))
            orig_answer = row['answer']
            answer1 = lower_all(nltk.word_tokenize(row['answer']))
...
def get_total_words(summaries, qaps_file):
    total_words = []
    with open(qaps_file, 'r') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL, skipinitialspace=True)
        next(spamreader)
        for row in spamreader:
            ques = nltk.word_tokenize(row[2])
            answ = nltk.word_tokenize(row[3])
            answ_2 = nltk.word_tokenize(row[4])
    ...
    for k,v in summaries.items():
        ...
        total_words.extend(w.lower() for w in v)
...
def create_processed_dataset(config, data_type):
    ...
    data_summary_path = os.path.join(
            config.data_dir, 'third_party', 'wikipedia', 'summaries-small.csv')
    data_input_path = os.path.join(config.data_dir, 'qaps-small.csv')
    relations = CG.get_relations(config.commonsense_file)

    summaries = {}

    with open(data_summary_path, 'r') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL, skipinitialspace=True)
        next(spamreader)
        ...
        for row in tqdm(spamreader):
            ...
            doc_num = row[0]
            tokenized_summary = nltk.word_tokenize(row[3].lower())
            summaries[doc_num] = tokenized_summary

    total_words = get_total_words(summaries, data_input_path)
    stop_words = get_stop_words(total_words, config.num_stop_words)     # 返回频率出现最多的45个词，不包括物主代词
    ...
    with open(data_input_path, 'r') as csvfile:
        ...
        for row in tqdm(spamreader):
            ...
            ques = nltk.word_tokenize(row[2].lower())
            answ = nltk.word_tokenize(row[3].lower())
            answ_2 = nltk.word_tokenize(row[4].lower())
